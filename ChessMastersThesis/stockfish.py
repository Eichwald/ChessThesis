# the code in this file is originally from 'https://github.com/zhelyabuzhsky/stockfish'
# but this version contains some bugfixes and additions, which is marked in code
...
class Stockfish:
	...
	def makeMove(self, move):
		...
		if self.is_move_correct(move):
			self._put(f"position fen {fen} moves {move}")
	
    # the following function is custom			
	def availableMoves(self): 
		self._put(f"go depth 1 searchmoves")
		text = self._read_line()
		print(text)
		text = self._read_line()
		print(text)
    ...
    # the following function contains a bug fix, as `depth` was not sent 
